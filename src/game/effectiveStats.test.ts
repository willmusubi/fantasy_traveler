...
describe('effectiveStats', () => {
  ...
  it('adds equipped item bonuses for THIS character only', () => {
    ...
    expect(effectiveStats(c, ctx).atk).toBe(20 + 6)
    expect(effectiveStats(c, ctx).spd).toBe(16 + 3)
  })
  ...
  it('applies party buff percentages for def/spd/mag (atk stays in the attack mult)', () => {
    const c = char('arc', 'arcanist') // base def 7, spd 11, mag 20, atk 7
    const ctx = {
      ownedEquipment: [],
      activeSynergies: [] as SynergyDef[],
      partyBuffs: [
    ...
    expect(s.mag).toBe(Math.round(20 * 1.25))
    expect(s.def).toBe(Math.round(7 * 1.2))
    expect(s.atk).toBe(7) // atkPct is NOT applied in effectiveStats
  })
